...
<main class="login-page">
    <section class="login-panel">
        <form class="login-form" method="post" action="{{ route('login.store') }}">
            ...
            <p class="lead" style="margin-bottom:28px">会社から発行されたログインIDでログインしてください。</p>
            @if($errors->any())<div class="alert alert-error">{{ $errors->first() }}</div>@endif
            <div class="field" style="margin-bottom:18px"><label for="login_id">ログインID</label><input id="login_id" name="login_id" value="{{ old('login_id') }}" autocomplete="username" required autofocus></div>
