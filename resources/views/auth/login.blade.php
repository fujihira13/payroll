...
<main class="login-page">
    <section class="login-panel">
        <form class="login-form" method="post" action="{{ route('login.store') }}">
            ...
            <p class="lead" style="margin-bottom:28px">会社から登録されたメールアドレスでログインしてください。</p>
            @if($errors->any())<div class="alert alert-error">{{ $errors->first() }}</div>@endif
            <div class="field" style="margin-bottom:18px"><label for="email">メールアドレス</label><input id="email" name="email" type="email" value="{{ old('email') }}" autocomplete="email" required autofocus></div>
