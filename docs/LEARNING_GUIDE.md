...
2. `app/Enums/UserRole.php` — 3つの権限を確認する
3. `app/Http/Middleware/EnsureRole.php` — 権限が違う利用者を拒否する仕組みを読む
4. `database/migrations` — どのデータを保存しているかを読む
5. 各Controller — 入力を検証し、Modelを操作し、Viewを返す流れを読む
6. `resources/views` — Bladeが受け取ったデータをHTMLにする部分を読む
7. `app/Services` — CSV取込と予約公開という少し複雑な処理を読む
8. `tests/Feature` — システムに期待する動作を具体例で読む
...
ブラウザ
  ...
  → auth と role:employee ミドルウェアが本人・権限を確認
...
- `system_admin` は会社に属さず、会社・企業管理者・共通テンプレートを扱います。
- `company_admin` は必ず1社に属し、すべての検索にログイン中利用者の `company_id` を付けます。
- `employee` も1社に属し、さらに明細の `employee_id` が自分自身であることを確認します。

URLを隠すだけでは安全になりません。そのため、このアプリはルートの `role` ミドルウェアとController内の所有者確認の両方を使っています。

## テンプレートと会社設定を分けた理由

システム管理者のテンプレートは「原本」です。企業管理者が選ぶと、項目を会社用設定へコピーします。コピー後は、会社側が項目名や並びを変えても原本や他社に影響しません。給与明細を作った時点でも項目をJSONとして保存するため、将来テンプレートが変わっても過去明細は変化しません。

## 承認・予約公開・メール

給与処理には `draft`、`scheduled`、`published` の状態があります。

- `draft`: CSVを取り込み、企業管理者が内容を確認中
