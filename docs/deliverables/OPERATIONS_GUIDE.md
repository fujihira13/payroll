...
最後のコマンドでは、最初のシステム管理者の氏名、ログインID、8文字以上のパスワードを入力します。パスワード入力中に文字が表示されないのは正常です。

初期データ用Seederは空です。会社、初期社員管理者、部署、社員、明細テンプレートは画面から登録します。
...
システム管理者は`admins`の専用認証を使います。`/login`では同じ社員アカウントを使い、`permission=1`を一般社員、`permission=9`を社員管理者として画面と操作権限を切り替えます。

## 5. 初期設定と基本運用

1. システム管理者が会社を登録する。
2. 会社登録時に、最初の社員管理者（permission 9）を同時登録する。
3. システム管理者が共通の給与明細テンプレートを登録する。
4. 社員管理者が部署と社員を登録する。
5. 社員管理者が共通テンプレートを自社用設定へコピーし、項目を調整する。
6. 社員管理者が通知メール文面を設定する。
7. 社員管理者が給与処理を作成し、画面から見本CSVを取得する。
8. 給与CSVを取り込み、内容を確認する。
9. 公開日時を指定して承認する。
10. Schedulerが公開時刻に明細を公開し、社員へログインURLを通知する。
11. 社員が明細を閲覧またはPDF出力する。
12. 社員管理者が未閲覧・閲覧日時・閲覧回数を確認する。
